...
(async () => {
  ...
  try {
    ...
    console.log('\n--- Step 2: Going to Document Viewer ---');
    ...
    await takeScreenshot(page, '03-dropdown-visible');
    
    // Click Create Flashcards and wait for navigation
    ...
    await takeScreenshot(page, '04-flashcard-viewer-after-creation');
    const newUrl = page.url();
    console.log(`Navigated to: ${newUrl}`);
    
    console.log('\n--- Step 4: Verifying on List Page ---');
    await page.goto('http://localhost:4200/flashcards');
    await page.waitForSelector('text=Flashcards for "Intro to Neural Networks"', { timeout: 10000 });
    await takeScreenshot(page, '05-flashcards-list-view');
    ...
    await takeScreenshot(page, '06-study-card-flipped');

    // Go to next card
    await page.click('button:has-text("Next")');
    await takeScreenshot(page, '07-study-next-card');
